...
| `AGENT_OPENCODE_URL` (+`_MODEL`) | — | Provedor delegado `opencode`, serviço `agent-cli`. Sem a URL o container sobe e ninguém o usa |
| `AGENT_CODEX_VISION` / `AGENT_OPENCODE_VISION` | on | Se o modelo que roda dentro do CLI enxerga imagem. Decisão do deployment: negue para um CLI de texto, senão todo passo pede captura |
