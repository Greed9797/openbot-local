...
| 7 | `active` never asserted in the History (BH-03) | The History row uses `active` to decide whether Continue may reopen a conversation, and `listBotConversations` computed it from its own pass over `agentRows` — no test read the field at all, in either state. Pinned by "a soft-deleted bot leaves its conversations inactive" (`server/tests/bot-history.test.ts`): `active: true` with `agentIds` before, `active: false` with the same `agentIds` after `profileStore.softDelete`. Asserted at the store because the route's guard is `profileStore.get`, which skips deleted profiles and answers 404 before reaching the branch. Bites: `active: row.deletedAt === null` → `active: true` fails it — 21 pass / 1 fail, restored to 22 pass. Also settles the `agents?.active ?? false` fallback at `routes.ts:428`: unreachable through the route, because every channel on a page has a `channelAgents` row for the bot (the `WHERE` subquery) and the bot has a profile (the route's own precondition), so `agentRows` always yields at least that row. `get` stays in agreement — `channelAgents.agentId` and `agentProfiles.agentId` both cascade from `agents.id`, so a channel cannot retain a link to an agent whose profile is gone. |
| 8 | `active` false was pinned only on the unreachable branch (BH-03) | Finding 7's test deletes the bot being read, which the route refuses at its own guard — so it pinned the store, and left the accumulation `entry.active &&= row.deletedAt === null` (`routes.ts:412`) untested. The way `active: false` actually reaches the UI is a dead *companion*: the bot is alive, the route answers 200, and the flag falls because it is an AND over every agent of the conversation. Pinned by "a soft-deleted companion turns the row inactive over HTTP" (`server/tests/bot-history.test.ts`): a two-agent hidden channel reads `active: true`, then `profileStore.softDelete(colega)` and the same HTTP call reads 200 with both `agentIds` and `active: false`. Bites: `&&=` → `\|\|=` fails it — 22 pass / 1 fail, restored to 23 pass. |
