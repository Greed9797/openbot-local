...
/*
 * O reforço: a resposta de memória deixa de ser só marcada e passa a ser refeita.
 *
 * O aviso honesto continuava deixando na tela a resposta errada — quem perguntou lia o valor
 * inventado primeiro e a ressalva depois. Medido no smoke e na bateria, a regra no prompt e o
 * AGENTS.md reduzem mas não eliminam: o modelo é quem decide chamar a ferramenta. Quando o turno
 * termina com pergunta que cita endereço, resposta dada e nenhuma chamada, este turno roda uma
 * segunda vez — e só uma — com a ordem explícita de abrir. Uma passagem custa segundos; um valor
 * inventado apresentado como lido custa confiança.
 */
export function turnoMereceReforço(
  pergunta: string,
  usouFerramenta: boolean,
): boolean {
...
export const REFORÇO_DE_LEITURA = [
  "Sua resposta anterior para esta mensagem chegou SEM abrir página nenhuma, e está reprovada: pergunta que cita endereço se responde lendo, não de memória.",
  "Abra agora o endereço citado usando a ferramenta de navegador e responda de novo com o que ele mostra hoje.",
  "Não reaproveite nem confirme a resposta anterior. Se a ferramenta falhar ou recusar, diga isso claramente em vez de responder de cabeça.",
].join("\n");

export function promptDeReforço(promptOriginal: string): string {
  return `${promptOriginal}\n\n${REFORÇO_DE_LEITURA}`;
}

/** O que a pessoa vê quando o reforço dispara, entre a resposta reprovada e a refeita. */
export const REPROVAÇÃO_DE_MEMÓRIA =
  "\n\n---\n_A resposta acima veio sem leitura nenhuma e foi reprovada. Abrindo o endereço agora:_\n\n";
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        ...
        /*
         * Uma passagem é um `codex exec` inteiro: nasce, transmite e é recolhida aqui dentro.
         *
         * Extraída para o reforço poder pedir uma segunda passagem sem duplicar o encanamento — o
         * estado que importa por passagem (chamou ferramenta? respondeu? o quê?) vem no retorno,
         * e o que a pessoa vê continua saindo pelo mesmo `say` na ordem em que acontece.
         */
        const executarPassagem = async (
          textoDaPassagem: string,
        ): Promise<{
          usouFerramenta: boolean;
          respondeu: boolean;
          abertos: string[];
          ...
          try {
            child = Bun.spawn(
              ...
              {
                stdin: new TextEncoder().encode(textoDaPassagem),
                stdout: "pipe",
                stderr: "pipe",
                env: {
                  ...process.env,
            ...
            return { usouFerramenta, respondeu: answered, abertos };
          } finally {
        ...
        /*
         * Duas passagens no máximo, e a segunda só existe num caso: pergunta que cita endereço,
         * resposta dada, nenhuma ferramenta chamada. O reforço é a diferença entre marcar a resposta
         * errada e substituí-la.
         */
        let resultado = await executarPassagem(prompt);
        if (
          COMPUTER_TOOLS &&
          resultado.respondeu &&
          ...
          resultado = {
            ...
            abertos: [...resultado.abertos, ...segunda.abertos],
          };
        }

        if (resultado.respondeu) {
          // Os avisos falam do estado FINAL do turno, com as duas passagens somadas.
          if (resultado.usouFerramenta) {
            // Ligado aqui depois de existir só em teste: ver o incidente do domínio parecido.
            const aviso = avisoDeOutraPagina(
              perguntaDoTurno(input),
              resultado.abertos,
            );
            if (aviso) say(aviso);
          } else {
            const aviso = avisoDeNaoLeitura(perguntaDoTurno(input), false);
            if (aviso) say(aviso);
          }
        }
      } catch (error) {
